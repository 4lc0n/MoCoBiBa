...
data[0] = device_adress << 1 | read|write
data[1 .... ]: daten zu übertragen
bei write: register + inhalt
bei read: ab data[1]: inhalt des ausgewählten Registers

  usiTwiStartTransceiverWithData(data, 2);
  mit length: länge des Arrays == (register+inhalt) + 1 bzw (daten_zu_lesen) +1
